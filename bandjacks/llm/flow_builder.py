...
class FlowBuilder:
    ...
    def _get_current_attack_version(self, session) -> str:
        """Get the current ATT&CK version in use. Delegates to FlowPersistence."""
        return self.persistence._get_current_attack_version(session)

    def persist_to_neo4j(self, flow_data: Dict[str, Any]) -> bool:
        """Persist flow to Neo4j. Delegates to FlowPersistence."""
        return self.persistence.persist_to_neo4j(flow_data)

    def generate_flow_embedding(self, flow_data: Dict[str, Any]) -> Dict[str, Any]:
        """Generate embedding for flow. Delegates to FlowPersistence."""
        return self.persistence.generate_flow_embedding(flow_data)
    ...
    def export_to_stix_attack_flow(
        ...
        Delegates to AttackFlowExporter.export_to_stix_attack_flow.
        ...
        """
        return self.exporter.export_to_stix_attack_flow(flow_data, scope=scope, marking_refs=marking_refs)

    def _lookup_technique(self, technique_id: str) -> Dict[str, Any]:
        """
        Look up technique details from Neo4j.

        Delegates to AttackFlowExporter._lookup_technique.
        ...
        Returns:
            Technique info dict
        """
        return self.exporter._lookup_technique(technique_id)
